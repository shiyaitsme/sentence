...
**Tag filter state**: tag filtering is split into two independent facets — `selectedCountryTags` and `selectedTopicTags` — plus `openFilterGroup` (`'country' | 'topic' | null`) tracking which one is expanded (the two facets expand exclusively; selections within each are independent of the other). `src/lib/tagGroups.js` classifies each tag from `tagCounts` into a facet via `splitTagGroups` — there's no DB field distinguishing a "country/period" tag from a plain topic tag (see D1 schema below), so classification is a static curated keyword list (`COUNTRY_PERIOD_TAGS`); anything not in that list falls into the topic facet by default. Filtering itself still ANDs every selected tag across both facets against `excerpt.tags` — the two-facet split is presentation only, not a change in match semantics.
...
- **`ExcerptListPage`** (`'list'`) — "摘抄集": title + "+ 新建摘抄", a two-level filter bar, and a CSS `columns`-based waterfall of `ExcerptCard`s. The filter bar is a "全部" pill plus two expandable facet triggers — "国家 / 时期" and "标签" (see Tag filter state above) — only one expands at a time, clicking a trigger reveals its facet's chips (`.filter-subchip`) below the row; a search box sits at the row's right edge. Clicking a card opens `ExcerptDetail` as a modal (view/edit/delete).
