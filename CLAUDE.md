...
- `public/icons/icon-*.png` are generated (not hand-drawn) from `public/icon_v3.png` (a four-leaf clover mark, transparent PNG) composited onto the `--card` cream color at each size — see the one-off script used to make them if the source art or sizes ever change (resize the mark with padding, flatten onto an opaque square; iOS icons should not be transparent). The same `icon_v3.png` also backs the browser-tab favicon in `index.html` (bump the `?v=` query param when the art changes, since it's otherwise cached hard).
